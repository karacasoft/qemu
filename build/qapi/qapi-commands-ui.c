...
static void qmp_marshal_output_DisplayOptions(DisplayOptions *ret_in, QObject **ret_out, Error **errp)
{
    Error *err = NULL;
    Visitor *v;

    v = qobject_output_visitor_new(ret_out);
    visit_type_DisplayOptions(v, "unused", &ret_in, &err);
    if (!err) {
        visit_complete(v, ret_out);
    }
    error_propagate(errp, err);
...
void qmp_marshal_query_display_options(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_DisplayOptions(retval, ret, &err);
