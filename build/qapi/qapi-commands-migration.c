...
static void qmp_marshal_output_COLOStatus(COLOStatus *ret_in, QObject **ret_out, Error **errp)
{
    Error *err = NULL;
    Visitor *v;

    v = qobject_output_visitor_new(ret_out);
    visit_type_COLOStatus(v, "unused", &ret_in, &err);
    if (!err) {
        visit_complete(v, ret_out);
    }
    error_propagate(errp, err);
...
void qmp_marshal_query_colo_status(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_COLOStatus(retval, ret, &err);
...
void qmp_marshal_migrate_recover(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_q_obj_migrate_recover_arg_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
