...
void visit_type_QCryptoBlockLUKSKeyslotState(Visitor *v, const char *name, QCryptoBlockLUKSKeyslotState *obj, Error **errp)
{
    int value = *obj;
    visit_type_enum(v, name, &value, &QCryptoBlockLUKSKeyslotState_lookup, errp);
    *obj = value;
}

void visit_type_QCryptoBlockAmendOptionsLUKS_members(Visitor *v, QCryptoBlockAmendOptionsLUKS *obj, Error **errp)
{
    Error *err = NULL;

    visit_type_QCryptoBlockLUKSKeyslotState(v, "state", &obj->state, &err);
    if (err) {
        goto out;
    }
    if (visit_optional(v, "new-secret", &obj->has_new_secret)) {
        visit_type_str(v, "new-secret", &obj->new_secret, &err);
        if (err) {
            goto out;
        }
    }
    if (visit_optional(v, "old-secret", &obj->has_old_secret)) {
        visit_type_str(v, "old-secret", &obj->old_secret, &err);
        if (err) {
            goto out;
        }
    }
    if (visit_optional(v, "keyslot", &obj->has_keyslot)) {
        visit_type_int(v, "keyslot", &obj->keyslot, &err);
        if (err) {
            goto out;
        }
    }
    ...
    if (visit_optional(v, "secret", &obj->has_secret)) {
        visit_type_str(v, "secret", &obj->secret, &err);
        if (err) {
            goto out;
        }
    }
...
void visit_type_QCryptoBlockAmendOptionsLUKS(Visitor *v, const char *name, QCryptoBlockAmendOptionsLUKS **obj, Error **errp)
{
    Error *err = NULL;

    visit_start_struct(v, name, (void **)obj, sizeof(QCryptoBlockAmendOptionsLUKS), &err);
    if (err) {
        goto out;
    }
    if (!*obj) {
    ...
    visit_type_QCryptoBlockAmendOptionsLUKS_members(v, *obj, &err);
    if (err) {
        goto out_obj;
    }
    visit_check_struct(v, &err);
...
void visit_type_QCryptoBlockAmendOptions_members(Visitor *v, QCryptoBlockAmendOptions *obj, Error **errp)
{
    Error *err = NULL;
    ...
    switch (obj->format) {
    case Q_CRYPTO_BLOCK_FORMAT_LUKS:
        visit_type_QCryptoBlockAmendOptionsLUKS_members(v, &obj->u.luks, &err);
        break;
    case Q_CRYPTO_BLOCK_FORMAT_QCOW:
        break;
    default:
...
void visit_type_QCryptoBlockAmendOptions(Visitor *v, const char *name, QCryptoBlockAmendOptions **obj, Error **errp)
{
    Error *err = NULL;

    visit_start_struct(v, name, (void **)obj, sizeof(QCryptoBlockAmendOptions), &err);
    if (err) {
        goto out;
    }
    if (!*obj) {
    ...
    visit_type_QCryptoBlockAmendOptions_members(v, *obj, &err);
    if (err) {
        goto out_obj;
    }
    visit_check_struct(v, &err);
