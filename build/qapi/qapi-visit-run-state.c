...
void visit_type_ShutdownCause(Visitor *v, const char *name, ShutdownCause *obj, Error **errp)
{
    int value = *obj;
    visit_type_enum(v, name, &value, &ShutdownCause_lookup, errp);
    *obj = value;
}
...
void visit_type_q_obj_SHUTDOWN_arg_members(Visitor *v, q_obj_SHUTDOWN_arg *obj, Error **errp)
{
    ...
    visit_type_ShutdownCause(v, "reason", &obj->reason, &err);
    if (err) {
        goto out;
    }
...
void visit_type_q_obj_GUEST_CRASHLOADED_arg_members(Visitor *v, q_obj_GUEST_CRASHLOADED_arg *obj, Error **errp)
{
    Error *err = NULL;
