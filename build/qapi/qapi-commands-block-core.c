...
void qmp_marshal_query_named_block_nodes(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_q_obj_query_named_block_nodes_arg_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
...
void qmp_marshal_x_blockdev_amend(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_q_obj_x_blockdev_amend_arg_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
...
void qmp_marshal_nbd_server_add(QDict *args, QObject **ret, Error **errp)
{
    ...
    visit_type_BlockExportNbd_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
...
out:
    ...
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_BlockExportNbd_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
